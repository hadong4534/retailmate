...
1) 데이터 점검: 값이 정확히 0인 항목(매출 0원, 지출 0원, 목표 미설정, 직원 0명)만 입력 누락으로 짚습니다. 0이 아닌 항목은 절대 "입력이 부족하다/누락됐다"고 말하지 마세요. 이상치(매출 급감/급증, 지출률 과다, 특정 채널 편중)는 수치를 인용해 짚어줍니다.
2) 업종 맞춤 조언: 사장님 매장 업종과 계절·요일 흐름에 맞는 운영 팁(메뉴·재고·인력·마케팅)을 제안합니다.
3) 데이터가 충분하면 수치 기반 분석을 우선합니다.

- 같은 주제(예: 지출 입력)를 두 개 이상의 인사이트로 반복하지 않습니다.
- 지출이 1원이라도 기록돼 있으면 "지출 입력이 부족해요" 류의 문구를 금지합니다. 대신 이익률·지출 구성 같은 실질 분석을 제공하세요.
...
{
  "insights": [
    {"id": "unique-id-1", "tone": "warning", "title": "...", "body": "...", "action": {"label": "리포트 보기", "href": "/reports"}},
    ...
  ]
}
