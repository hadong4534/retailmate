...
export function generateExpenseInsight(input: ExpenseInsightInput): ExpenseInsight {
  if (input.monthExpenses === 0) {
    return {
      title: '아직 입력된 지출이 없어요',
      body: '지출을 입력해야 정확한 영업이익이 계산됩니다.',
  ...
  if (input.prevMonthExpenses > 0) {
    ...
    if (delta <= -5) {
      ...
      return {
        title: '전월 대비 지출이 감소했어요!',
        body: `이번 달 지출은 전월 대비 ${Math.abs(delta).toFixed(1)}% 감소했어요.\n${cutText}`,
        tip: { text: '식자재 발주량을 최적화하면 원재료비를 추가로 절감할 수 있어요.' },
      };
    }
    if (delta >= 10) {
      return {
        title: `전월 대비 지출이 ${delta.toFixed(1)}% 늘었어요.`,
        body: '카테고리별 변동을 확인하고 지출 폭증의 원인을 점검해보세요.',
  ...
  if (laborPct >= 40) {
    return {
      title: '인건비 비중이 높습니다',
      body: `이번 달 지출 중 인건비가 ${laborPct.toFixed(1)}%를 차지해요. 외식업 평균(28%) 대비 다소 높은 편입니다.`,
  ...
  if (expenseRatio > 0) {
    if (expenseRatio >= 90) {
      return {
        title: '지출률이 매우 높아요',
        body: `매출 대비 지출 ${expenseRatio.toFixed(1)}%. 영업이익이 거의 남지 않는 수준입니다.`,
    ...
    if (expenseRatio <= 50) {
      return {
        title: '안정적인 지출 구조입니다',
        body: `매출 대비 지출 ${expenseRatio.toFixed(1)}%. 건전한 운영 상태예요.`,
  ...
  if (input.topCategory) {
    return {
      ...
      body: `이번 달 ${input.topCategory.label}에 ₩${input.topCategory.amount.toLocaleString('ko-KR')} 사용. 전체 지출의 큰 비중을 차지합니다.`,
  ...
  return {
    title: '지출 분석',
    body: `이번 달 지출 합계 ₩${input.monthExpenses.toLocaleString('ko-KR')}.`,
  };
}
